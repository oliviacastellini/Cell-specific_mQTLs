...
for(n in 1:length(cell_types)) {
  ...
  beta_values <- lapply(cell_type_data, function(df) df$beta)

  # Calculate the pairwise correlation between beta values
  correlations <- cor(beta_values[[1]], beta_values[[2]])

  # Store the correlation result in the list
  correlation_results[[cell_types[n]]] <- correlations
}

# Print the correlation results
for (n in 1:length(cell_types)) {
  cat("Cell Type:", cell_type, "\n")
  cat("Correlation between models:", correlation_results[[cell_type[n]]], "\n")
}
```
