...
for (i in 1:length(cell_types)) {
    # Extract data for the current cell type from both models
  for (n in 1:7) {
...
model1_merged$beta<-as.numeric(as.character(model1_merged$beta))
  ...
  for (n in 1:7) {
     model2<-read.table(paste0("/user/home/uy23281/scratch/rmd.results/model2/",cell_types[i],"/res.cis.", n, ".gz"))
     if (n == 1) {
      model2_merged <- model2
    } else {
      model2_merged <- rbind(model2_merged, model2)
    }
  }
model2_merged<-model2_merged[-1,]
colnames(model2_merged)<-c("SNP","cpg","beta","t","P","FDR")
...
# Count overlapping significant mQTLs
    significant_overlap[i] <- sum(model1_merged$P < 0.05 & model2_merged$P < 0.05)
    
# Plot
model1_merged<-model1_merged[order(model1_merged$P),]
model2_merged<-model2_merged[order(model2_merged$P),]
